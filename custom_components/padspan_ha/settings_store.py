...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    "door_composites": [],  # receipt for Devices -> Door Openers' "build from relays" wizard: [{id, kind, name, entity_id, relays, generated:{scripts,automations,helper_id,template_entry_id}}] — every HA object the wizard created for one composite, so Remove can delete exactly those and nothing else; Bright+ control, tier-blind storage
    ...
    "lights_automorph_style": "glow",   # which visual treatment/shape the fixture's aura takes — see AUTOMORPH_STYLE_LABELS in iso_lights.js for the full, single-source-of-truth list
    ...
    "atlas_layout_v2": True,  # Garry, 2026-09-21: shipped as the default Atlas layout after the trial (still a toggle — "▦ Classic layout" reverts it) — see layoutTierFor in views/lights_map.js
