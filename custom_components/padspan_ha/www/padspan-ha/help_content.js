...
export const HELP = {
  ...
  overview_grid: {
    ...
    body: [
      "Once your rooms have real geometry (floor plans traced in Mapping), Overview draws a proper isometric floor stack — each room an extruded slab in its own colour, stacked by floor. Rooms that share a wall blend into one surface rather than each getting its own outline.",
  ...
  monitor: {
    ...
    body: [
      ...
      "Health — a live System Critics summary, the same automated diagnosis described below. Diag Export — export the current diagnostic state as JSON. Debug — a raw dump of the panel's internal state, for troubleshooting.",
    ],
  },
  zones: {
  ...
  insights: {
    title: "Insights — Visual analytics",
    body: [
      ...
      "Further down, additional cards cover object mobility (devices seen moving between rooms), coverage gaps, and overall device breakdown stats.",
  ...
  qa: {
    ...
    body: [
      ...
      "Radio Analysis — a full section ranking every scanner by hardware, coverage, and reliability. See its own help for the scoring breakdown.",
  ...
  overview_3d_controls: {
    ...
    body: [
      ...
      "Followed (starred) devices always draw as an amber beacon marker, whether this row is on or off — they just dim when away. What the Pins button controls is everything else: ON draws every other identified/labelled device as a red crosshair (away) or teal dot (present); OFF instead shows only unlabelled-but-identified devices, dimmed.",
  ...
  manage_data: {
    ...
    body: [
      ...
      "BACKUP & RESTORE — Creates/restores a complete backup of every PadSpan store: settings, calibration, adaptive model, objects, maps config, model (fixture/scanner placements in metres), alerts, movement history, traceback, and object history. Maps and Model are checked as a pair — a map's real-world placements live in Model, keyed to that map, so restoring one without the other would leave positions pointing at nothing. Always back up before making major changes.",
  ...
  calibration_overview: {
    ...
    body: [
      ...
      "ROAM — A guided coverage walk: PadSpan shows you a coverage heatmap and a 'next target' crosshair for the weakest-covered spot, and you press 'I'm Here — Collect' at each one — the same discrete collection Pin & Listen uses, just guided to where you need more data instead of you picking spots yourself.",
  ...
  bluetooth_overview: {
    ...
    body: [
      ...
      "VISUALISATION — Interactive graph showing how scanners and devices are connected. Line and node colour reflect signal strength (green/amber/red); node size is fixed. Click a scanner node to open its detail modal — that's also where you assign it to a room.",
      ...
      "SCANNERS — Lists all Bluetooth receivers/scanners with their area assignment, online status, and other details. Click a scanner row to see the devices it's currently hearing in the panel beside it. To change which room a scanner belongs to, open its detail modal from the Visualisation tab instead.",
