...
export const SHOWCASE_THEMES = {
  ...
  neo_hud: {
    // Reworked 2026-09-16 (Garry: "make them as different as you can"). It
    // used to be a fourth dark-navy-plus-amber theme, a 0.34 similarity
    // score from Art Deco — the amber accent hid the one thing it had of
    // its own, a cyan gloss. Now it IS the cyan: the only theme whose room
    // edges are a bright colour rather than a dark line, the widest label
    // tracking in the set, near-flat fills and the tightest light pools —
    // linework and readouts, a heads-up display, not a lit interior.
    label: "Neo HUD",
    vignetteStops: [[0,"#04151c",0.7], [60,"#020b10",0.35], [100,"#000000",0]],
    slabStops: [[0,"#0a2a36",0.10], [100,"#000000",0.22]],
    slabSideTop: {fill:"#061a22", fillOpacity:0.9, stroke:"#1fd9ff"},
    slabSideFront: {fill:"#03111a", fillOpacity:0.92, stroke:"#0d7f99"},
    glossStops: [[0,"#8febff",0.18], [45,"#4fe3ff",0.06], [100,"#000",0.3]],
    washStops: [[0,"#4fe3ff",0.10], [100,"#4fe3ff",0]],
    shadeStops: [[0,"#000",0.65], [55,"#000",0.28], [100,"#000",0]],
    roomEdgeStroke: "#1fd9ff",
    roomFillOpacity: 0.04,
    roomStrokeOpacity: 0.95,
    roomStrokeWidth: 1.0,
    roomGlowStops: [[0,0.30], [35,0.10], [100,0]],
    roomLabelOpacity: 0.95,
    roomLabelLetterSpacing: "0.28em",
    roomLabelUppercase: true,
    poolStops: [[0,0.85], [18,0.35], [40,0.08], [100,0]],
    fixtureOffFill: "#0a1a20",
    fixtureOnStrokeFallback: "#4fe3ff",
    fixtureOffStroke: "#1a4a58",
    fixtureBloomOpacity: 0.45,
    fixtureBodyOnOpacity: 0.95,
    fixtureBodyOffOpacity: 0.35,
    inkOffColor: "#2d5f6e",
    labelColorOff: "#3f8a9e",
    codeChipBg: "#020c10",
  ...
  organic_bioluminescent: {
    // Second pass 2026-09-16: a dark teal with a near-white accent and hard
    // pools — 0.73 from Automotive HUD, of all things, on structure. Its
  ...
  neon_precision: {
    // Second pass 2026-09-16: "Bold Neon" had a near-white accent and dark
    // room edges — nothing neon about it, 0.56 from Art Deco. Now it is the
    // neon-tube theme: electric-violet room outlines (the only violet edge),
    // acid-green lit fixtures (the only saturated green accent — Aurora's is
    // a pale mint), almost no room fill because a neon sign is lines on
    // black, and the strongest outlines in the set.
    label: "Bold Neon Precision",
    vignetteStops: [[0,"#12061f",0.6], [55,"#080312",0.3], [100,"#000000",0]],
    slabStops: [[0,"#b026ff",0.05], [100,"#000000",0.18]],
    slabSideTop: {fill:"#0a0612", fillOpacity:0.5, stroke:"#7a1fd6"},
    slabSideFront: {fill:"#05030a", fillOpacity:0.45, stroke:"#4c1289"},
    glossStops: [[0,"#e9c6ff",0.22], [45,"#b026ff",0.08], [100,"#000",0.25]],
    washStops: [[0,"#b026ff",0.07], [100,"#b026ff",0]],
    shadeStops: [[0,"#000",0.65], [55,"#000",0.28], [100,"#000",0]],
    roomEdgeStroke: "#b026ff",
    roomFillOpacity: 0.02,
    roomStrokeOpacity: 0.98,
    roomStrokeWidth: 1.8,
    roomGlowStops: [[0,0.38], [40,0.12], [100,0]],
    roomLabelOpacity: 0.9,
    roomLabelLetterSpacing: "0.16em",
    roomLabelUppercase: true,
    poolStops: [[0,0.95], [22,0.55], [50,0.22], [100,0]],
    fixtureOffFill: "#0a0612",
    fixtureOnStrokeFallback: "#39ff14",
    fixtureOffStroke: "#3d2b5c",
    fixtureBloomOpacity: 0.6,
    fixtureBodyOnOpacity: 0.95,
    fixtureBodyOffOpacity: 0.4,
    inkOffColor: "#7a6a99",
    labelColorOff: "#8f7fb0",
    codeChipBg: "#07040d",
    codeChipBgOpacity: 0.8,
  },
  luxury_realestate: {
    // Reworked 2026-09-16. It was a fifth dark-ground-plus-gold theme (0.44
    // from Retro-Futurism, close to Art Deco too). Listing photography is
    // BRIGHT, so this is now the only gold-accent theme on a light ground —
    // ivory marble and metallic brass — and it is deliberately the SIMPLE
    // option Garry asked for: hairline strokes, next to no glow, the lowest
    // bloom in the set, a near-flat room fill. Hygge is the other warm light
    // theme; that one is kraft-paper tan with an orange accent and a heavy
    // fill — cosy. This is crisp.
    label: "Luxury Real-Estate",
    // Cool white marble, not warm ivory: warm cream is Wabi-Sabi's ground and
    // kraft tan is Hygge's, so the brass has to sit on a cool grey-white to
    // read as its own thing on the light shelf.
    vignetteStops: [[0,"#f7f8fa",0.97], [60,"#eef0f4",0.9], [100,"#dfe3ea",0.8]],
    slabStops: [[0,"#ffffff",0.5], [100,"#cfd4dc",0.3]],
    slabSideTop: {fill:"#e9ebef", fillOpacity:0.9, stroke:"#c9a227"},
    slabSideFront: {fill:"#d8dce3", fillOpacity:0.9, stroke:"#b08d1f"},
    glossStops: [[0,"#ffffff",0.3], [45,"#ffffff",0.06], [100,"#3a4250",0.06]],
    washStops: [[0,"#fff",0.03], [100,"#fff",0]],
    shadeStops: [[0,"#2a3140",0.14], [55,"#2a3140",0.05], [100,"#2a3140",0]],
    roomEdgeStroke: "#1f2530",
    roomFillOpacity: 0.03,
    roomStrokeOpacity: 0.6,
    roomStrokeWidth: 0.9,
    roomGlowStops: [[0,0.04], [45,0.01], [100,0]],
    roomLabelOpacity: 0.7,
    roomLabelLetterSpacing: "0.12em",
    roomLabelUppercase: true,
    poolStops: [[0,0.5], [30,0.24], [62,0.08], [100,0]],
    fixtureOffFill: "#f4efe6",
    fixtureOnStrokeFallback: "#c9a227",
    fixtureOffStroke: "#b8ad9c",
    fixtureBloomOpacity: 0.10,
    fixtureBodyOnOpacity: 0.9,
    fixtureBodyOffOpacity: 0.7,
    inkOffColor: "#8f8778",
    labelColorOff: "#a39a88",
    codeChipBg: "#ffffff",
    codeChipBgOpacity: 0.9,
  },
  wabi_sabi: {
    // Second pass 2026-09-16: 0.58 from Hygge — both warm paper, both with
    // a pale accent that barely shows a lit fixture. Hygge is the dense,
    // cosy one (heavy fill, opaque slabs, orange). This is now the sparse,
    // matte one: a cooler stone-grey washi, near-zero fill and gloss, thin
    // faint edges, lowercase labels, and a matcha accent — the only muted
    // olive-green lit fixture in the set (Aurora's mint and Neon's acid
    // green are both saturated).
    label: "Wabi-Sabi Zen",
    vignetteStops: [[0,"#f4efe4",0.85], [55,"#e8e1d0",0.5], [100,"#d3cbb8",0.2]],
    slabStops: [[0,"#f8f3e8",0.12], [100,"#b9b19f",0.14]],
    slabSideTop: {fill:"#d9d3c4", fillOpacity:0.5, stroke:"#6e6a5e"},
    slabSideFront: {fill:"#c4bcaa", fillOpacity:0.45, stroke:"#6e6a5e"},
    glossStops: [[0,"#fbf8f0",0.06], [45,"#fbf8f0",0.02], [100,"#332e26",0.08]],
    washStops: [[0,"#fbf8f0",0.04], [100,"#fbf8f0",0]],
    shadeStops: [[0,"#332e26",0.3], [55,"#332e26",0.12], [100,"#332e26",0]],
    roomEdgeStroke: "#3a362e",
    roomFillOpacity: 0.03,
    roomStrokeOpacity: 0.5,
    roomStrokeWidth: 1.0,
    roomGlowStops: [[0,0.06], [45,0.02], [100,0]],
    roomLabelOpacity: 0.65,
    roomLabelLetterSpacing: "0.1em",
    roomLabelUppercase: false,
    poolStops: [[0,0.6], [30,0.3], [65,0.12], [100,0]],
    fixtureOffFill: "#d6cfbf",
    fixtureOnStrokeFallback: "#8a9a5b",
    fixtureOffStroke: "#7a746a",
    fixtureBloomOpacity: 0.12,
    fixtureBodyOnOpacity: 0.85,
    fixtureBodyOffOpacity: 0.6,
    inkOffColor: "#6b6a62",
    labelColorOff: "#5f5d55",
    codeChipBg: "#f5f1e8",
    codeChipBgOpacity: 0.8,
  },
  hygge: {
  ...
  aurora: {
    // Reworked 2026-09-16. There was no aurora in it — plain dark purple
    // with a white accent, 0.53 from Holographic. Now the vignette IS the
    // curtain: green through teal into violet, a green-to-violet gloss and
    // a coloured wash, the only green fixture accent in the set. Built as
    // the soft, diffuse end of the range on purpose: the biggest room glow
    // and the widest light pools of any theme, lowercase labels — the
    // opposite of Neo HUD's hard linework.
    label: "Aurora",
    vignetteStops: [[0,"#0b3d2e",0.75], [40,"#1a6b5a",0.35], [70,"#3b1f6e",0.4], [100,"#050311",0]],
    slabStops: [[0,"#22d3a0",0.08], [100,"#0a0824",0.16]],
    slabSideTop: {fill:"#0f1f2e", fillOpacity:0.3, stroke:"#2a7a66"},
    slabSideFront: {fill:"#0a0824", fillOpacity:0.26, stroke:"#4c2a8a"},
    glossStops: [[0,"#7cf5c6",0.35], [45,"#a78bfa",0.18], [100,"#000",0.2]],
    washStops: [[0,"#5eead4",0.14], [100,"#a78bfa",0]],
    shadeStops: [[0,"#000",0.45], [55,"#000",0.18], [100,"#000",0]],
    roomEdgeStroke: "#0a0824",
    roomFillOpacity: 0.14,
    roomStrokeOpacity: 0.45,
    roomStrokeWidth: 1.2,
    roomGlowStops: [[0,0.55], [45,0.22], [100,0]],
    roomLabelOpacity: 0.8,
    roomLabelLetterSpacing: "0.06em",
    roomLabelUppercase: false,
    poolStops: [[0,0.7], [40,0.42], [75,0.16], [100,0]],
    fixtureOffFill: "#1a2337",
    fixtureOnStrokeFallback: "#7cf5c6",
    fixtureOffStroke: "#3d5a6e",
    fixtureBloomOpacity: 0.55,
    fixtureBodyOnOpacity: 0.85,
    fixtureBodyOffOpacity: 0.45,
    inkOffColor: "#6e8aa0",
    labelColorOff: "#8fb3c4",
    codeChipBg: "#050a14",
    codeChipBgOpacity: 0.55,
  },
  automotive_hud: {
    // Second pass 2026-09-16: near-black charcoal with a pale amber accent
    // put it 0.58 from Obsidian Noir and 0.55 from Art Deco. Its own idea was
    // always "brushed-alloy cockpit bezel" — so the ground is a mid-grey
    // metal now, the only grey ground in the set, with a high metallic
    // sheen, bezel-grey room edges rather than black, and a saturated
    // gauge-LED amber-orange instead of Obsidian's pale brass. It keeps the
    // one mechanic nobody else has: the vignette darkens OUTWARD (bright
    // centre, dark rim), plus the tight bright pools of a dial.
    label: "Automotive HUD",
    vignetteStops: [[0,"#3a3f47",0.15], [65,"#1e2228",0.2], [100,"#0b0d10",0.55]],
    slabStops: [[0,"#6b727c",0.08], [100,"#15181c",0.16]],
    slabSideTop: {fill:"#4a5059", fillOpacity:0.6, stroke:"#6b727c"},
    slabSideFront: {fill:"#33383f", fillOpacity:0.55, stroke:"#555b64"},
    glossStops: [[0,"#ffffff",0.5], [45,"#ffffff",0.12], [100,"#000000",0.3]],
    washStops: [[0,"#ffd9a3",0.035], [100,"#ffd9a3",0]],
    shadeStops: [[0,"#000000",0.55], [55,"#000000",0.22], [100,"#000000",0]],
    roomEdgeStroke: "#3a3f47",
    roomFillOpacity: 0.06,
    ...
    roomLabelUppercase: true,
    poolStops: [[0,0.95], [20,0.5], [45,0.14], [100,0]],
    fixtureOffFill: "#2b2f36",
    fixtureOnStrokeFallback: "#ff8c1a",
    fixtureOffStroke: "#5c636d",
    fixtureBloomOpacity: 0.4,
    fixtureBodyOnOpacity: 0.9,
    fixtureBodyOffOpacity: 0.6,
    inkOffColor: "#6e757f",
    labelColorOff: "#7d848e",
    codeChipBg: "#1e2228",
    codeChipBgOpacity: 0.75,
  },
  art_deco: {
    // Second pass 2026-09-16: the gallery has always promised "gold
    // linework" and the theme never drew any — its room edges were the same
    // near-black every dark theme uses, which is why it sat 0.55 from
    // Automotive HUD and 0.56 from Neon. The edges are metallic gold now,
    // the only gold outline in the set, on a deeper midnight than Cinematic
    // Glass's steel blue, keeping the heaviest room fill of any dark theme —
    // the jewel glass between the gold.
    label: "Art Deco Luxury",
    vignetteStops: [[0,"#0f2140",0.6], [55,"#081124",0.28], [100,"#02050b",0]],
    slabStops: [[0,"#e8d8a0",0.06], [100,"#02050b",0.14]],
    slabSideTop: {fill:"#1b140a", fillOpacity:0.34, stroke:"#b8962e"},
    slabSideFront: {fill:"#0c0906", fillOpacity:0.3, stroke:"#8f7322"},
  ...
  holographic: {
    // Second pass 2026-09-16: the gloss already shifted pink to lavender —
    // the one iridescent thing here — but the wash was a single flat blue.
    // The wash now runs pink through lilac to cyan too, so the whole sheet
    // reads as one pearlescent surface, and the slab edges go lilac to
    // match. Nothing else in the set has a multi-hue wash.
    ...
    slabSideTop: {fill:"#1c1530", fillOpacity:0.34, stroke:"#6b5aa8"},
    slabSideFront: {fill:"#130d22", fillOpacity:0.28, stroke:"#4d3f85"},
    glossStops: [[0,"#ffc2ea",0.55], [45,"#c9a6ff",0.25], [100,"#1a1030",0.22]],
    washStops: [[0,"#ff9ff3",0.10], [50,"#c3a3ff",0.06], [100,"#7cf0ff",0]],
  ...
  retro_futurism: {
    // Reworked 2026-09-16. Its sunset fade was the only thing keeping it off
    // Luxury Real-Estate (0.44) — a peach-to-lilac wash under yet another
    // gold accent. The fade is now the whole idea, pushed all the way:
    // hot orange through magenta into indigo, the only four-stop sunset in
    // the set, hot-pink room edges (a lit floor grid), the heaviest strokes
    // of any dark theme, and a lemon accent no other theme uses. Built as
    // the bold, high-contrast end of the range — Aurora is the soft one.
    label: "Elevated Retro-Futurism",
    vignetteStops: [[0,"#ff6b35",0.5], [40,"#ff2d95",0.32], [75,"#4a1d8a",0.3], [100,"#0d0521",0]],
    slabStops: [[0,"#ff9a5c",0.08], [100,"#0d0521",0.18]],
    slabSideTop: {fill:"#2a0f4a", fillOpacity:0.4, stroke:"#ff2d95"},
    slabSideFront: {fill:"#160a30", fillOpacity:0.36, stroke:"#b31ea0"},
    glossStops: [[0,"#ffd6c2",0.4], [45,"#ff7ac6",0.14], [100,"#0d0521",0.22]],
    washStops: [[0,"#ff5ab0",0.08], [100,"#ff5ab0",0]],
    shadeStops: [[0,"#0d0521",0.55], [55,"#0d0521",0.22], [100,"#0d0521",0]],
    roomEdgeStroke: "#ff2d95",
    roomFillOpacity: 0.10,
    roomStrokeOpacity: 0.9,
    roomStrokeWidth: 2.2,
    roomGlowStops: [[0,0.30], [45,0.10], [100,0]],
    roomLabelOpacity: 0.9,
    roomLabelLetterSpacing: "0.18em",
    roomLabelUppercase: true,
    poolStops: [[0,0.9], [28,0.5], [60,0.16], [100,0]],
    fixtureOffFill: "#1f0d3d",
    fixtureOnStrokeFallback: "#ffe66d",
    fixtureOffStroke: "#6b3f9a",
    fixtureBloomOpacity: 0.5,
    fixtureBodyOnOpacity: 0.9,
    fixtureBodyOffOpacity: 0.5,
    inkOffColor: "#a67cc9",
    labelColorOff: "#c98bd6",
    codeChipBg: "#0d0521",
    codeChipBgOpacity: 0.75,
  },
  obsidian_noir: {
...
// pulse, orbitring, puzzle and shatter cut on Garry's word — a side-by-side
// grid of every style at once showed they all read as "a soft blob with a
// different border decoration", the same silhouette every time, only the
// linework changed. Replaced with six genuinely different SILHOUETTES —
// each one reshapes the actual ring geometry (like spikecrown/scallop
// already did) rather than decorating the same shared outline, so the
// difference survives at real map-marker scale, not just in a full-screen
// close-up.
export const AUTOMORPH_STYLE_LABELS = {
  glow: "Glow", blueprint: "Blueprint", nebula: "Nebula",
  halo: "Halo", spikecrown: "Spike Crown", scallop: "Scalloped Wave",
  bloomflower: "Bloom", geode: "Geode Cluster", honeycomb: "Honeycomb",
  extrude: "Block Extrude",
  shardburst: "Shard Burst", origami: "Origami Fold", inkbleed: "Ink Bleed",
  rosette: "Rosette", lensflare: "Lens Flare", mycelium: "Mycelium",
};
...
export function shapeSvg(kind, cx, cy, r, attrs){
  ...
  switch(kind){
    ...
    // A humidity readout: a solid teardrop — the universal "water" glyph, a
    // pointed top fused into a round bulb. Same technique as lock's
    // shackle+body and door's leaf+handle: two solid primitives overlapped
    // enough to read as one silhouette rather than two shapes.
    case "humidityreadout": {
      const bulbR=HW*0.40, bulbCy=cy+HW*0.14, apexY=cy-r*0.78, baseY=bulbCy-bulbR*0.5, baseW=bulbR*1.5;
      const drop=sub([[cx,apexY],[cx-baseW/2,baseY],[cx+baseW/2,baseY]]);
...
export function buildIsoSVG(model, byRoom, hiddenEids, focusZ, floorGap, horizGap, lightsByEid={}, lightsLoading=false, floors=[], opts={}){
  ...
  for(const z of levels){
    ...
    const markerSvg=(l,hx,hy,entry,extra="",suppressGlyph=false,nearestNeighborDist=Infinity,auraHitD=null)=>{
      ...
      // Live humidity readout — same placed+fresh gate as temperature
      // (Garry, 2026-09-15: "set it up like temperature"), no tint bands:
      // that was a temperature-specific request, not part of this one.
      const humidityFreshMs=l.isHumidity && l.last_changed ? NOW_MS-Date.parse(l.last_changed) : NaN;
      const humidityLive=!!(l.isHumidity && entry && Number.isFinite(l.humidity)
                        && humidityFreshMs>=0 && humidityFreshMs<TEMP_FRESH_MS);
      ...
      const tempLbl=tempLive
        ...
        : null;
      // Same live-readout replacement as temperature, one class down: a
      // fixed indigo (no tint bands — those were a temperature-specific
      // request), "%" suffixed.
      const humidityLbl=humidityLive
        ? `<text x="${hx.toFixed(1)}" y="${hy.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" `+
          `font-family="ui-monospace,monospace" font-size="${TEMP_DIGIT_PX.toFixed(1)}" font-weight="800" `+
          `fill="${HUMIDITY_BORDER}" paint-order="stroke" stroke="#050d09" stroke-width="${(TEMP_DIGIT_PX*0.32).toFixed(1)}" `+
          `stroke-linejoin="round" pointer-events="none">${l.humidity}%</text>`
        : null;
      ...
      const lbl=tempLbl!==null ? tempLbl : humidityLbl!==null ? humidityLbl : (HIDECODES ? (CODECHIP ? codeChipSvg(l,hx,hy,SHOW?tCol:"#e2e8f0",chipGap,true) : "") : (CODECHIP ? codeChipSvg(l,hx,hy,SHOW?tCol:"#e2e8f0",chipGap) : (SHOW
    ...
    const automorphAuraSvg=(l,hx,hy,room,z,cellPtsM,entry,hitOut)=>{
      ...
      // Six replacements for the 2026-09-17 cut (circuit, contour, facet,
      // sumie, stainedglass, constellation, pulse, orbitring, puzzle,
      // shatter — see AUTOMORPH_STYLE_LABELS's own comment for why): each
      // one reshapes the actual silhouette or composition, the same
      // discipline spikecrown/scallop/geode/honeycomb/extrude already
      // follow, rather than decorating the shared morphed ring with
      // different linework — a side-by-side comparison of the cut styles
      // showed that difference vanishes at real map-marker scale, while a
      // genuinely different SHAPE survives it.
      if(AUTOMORPH_STYLE==="shardburst"){
        // Several separate filled fragments scattered around the fixture
        // — never one continuous bounded ring, unlike every style above.
        // State is shard count, reach and brightness: off is a few small
        // slivers close to the fixture; on is more, bigger, brighter
        // shards reaching toward the ring itself.
        const n=ring.length;
        if(n<3) return {glow:"", edge:""};
        const shardCount = on ? 6+Math.round(2*t) : 4;
        const step = Math.max(1, Math.floor(n/shardCount));
        const widthSteps = Math.max(1, Math.round(step*(on?0.16:0.10)));
        const reach = on ? (0.55+0.45*t) : (0.30+0.20*t);
        const fillOp = opac((on?0.30:0.16)+0.22*t);
        const edgeOp = opac((on?0.55:0.30)+0.25*t);
        let shards="", edges="";
        for(let k=0;k<shardCount;k++){
          const i=(k*step)%n, j=(i+widthSteps)%n;
          const [ax,ay]=ring[i], [bx,by]=ring[j];
          const mx=hx+(ax-hx)*reach, my=hy+(ay-hy)*reach;
          const nx=hx+(bx-hx)*reach, ny=hy+(by-hy)*reach;
          const hxs=hx.toFixed(1), hys=hy.toFixed(1);
          const mxs=mx.toFixed(1), mys=my.toFixed(1), nxs=nx.toFixed(1), nys=ny.toFixed(1);
          shards+=`<path d="M${hxs},${hys} L${mxs},${mys} L${nxs},${nys} Z" fill="${duo}" fill-opacity="${fillOp}" pointer-events="none"/>`;
          edges+=`<path d="M${hxs},${hys} L${mxs},${mys} M${hxs},${hys} L${nxs},${nys} M${mxs},${mys} L${nxs},${nys}" `+
            `fill="none" stroke="${ink}" stroke-width="${swid(on?1.1:0.8)}" stroke-opacity="${edgeOp}" stroke-linecap="round" pointer-events="none"/>`;
        }
        return {glow: clipWrap(shards), edge: clipWrap(edges)};
      }
      if(AUTOMORPH_STYLE==="origami"){
        // A handful of large flat-colour panels fan-cut from the fixture's
        // own anchor — never many small facets (a different, cut style's
        // job) — alternating light/dark flat tones with no gradient
        // anywhere, so it reads as folded paper under flat light rather
        // than a lit surface. State is panel count and tone contrast.
        const n=ring.length;
        if(n<3) return {glow:"", edge:""};
        const panelCount = on ? 6 : 4;
        const step=Math.max(1, Math.floor(n/panelCount));
        const baseHex = on ? AUTOMORPH_BASE_ON : AUTOMORPH_BASE_OFF;
        const baseR=parseInt(baseHex.slice(1,3),16), baseG=parseInt(baseHex.slice(3,5),16), baseB=parseInt(baseHex.slice(5,7),16);
        const mixTo=(amt,to)=>[baseR,baseG,baseB].map((c,ci)=>Math.round(c+(to[ci]-c)*amt));
        const foldOp=opac(0.55+0.30*t);
        const fillOp=opac((on?0.30:0.16)+0.20*t);
        let panels="", folds="";
        for(let k=0;k<panelCount;k++){
          const i=(k*step)%n, j=((k+1)*step)%n;
          const rgb = (k%2===0) ? mixTo(0.45+0.15*t,[255,255,255]) : mixTo(0.45+0.15*t,[8,8,8]);
          const p0=`${ring[i][0].toFixed(1)},${ring[i][1].toFixed(1)}`, p1=`${ring[j][0].toFixed(1)},${ring[j][1].toFixed(1)}`;
          panels+=`<path d="M${hx.toFixed(1)},${hy.toFixed(1)} L${p0} L${p1} Z" fill="rgb(${rgb[0]},${rgb[1]},${rgb[2]})" fill-opacity="${fillOp}" pointer-events="none"/>`;
          folds+=`<line x1="${hx.toFixed(1)}" y1="${hy.toFixed(1)}" x2="${ring[i][0].toFixed(1)}" y2="${ring[i][1].toFixed(1)}" stroke="${ink}" stroke-width="${swid(0.9)}" stroke-opacity="${foldOp}" pointer-events="none"/>`;
        }
        const rim=`<path d="${d}" fill="none" stroke="${ink}" stroke-width="${swid(on?1.2:0.8)}" stroke-opacity="${opac((on?0.5:0.3)+0.2*t)}" pointer-events="none"/>`;
        return {glow: clipWrap(panels), edge: clipWrap(folds+rim)};
      }
      if(AUTOMORPH_STYLE==="inkbleed"){
        // An irregular, asymmetric bleed — the ring pushed outward by a
        // sum of deterministic sine waves at different frequencies from
        // the fixture's own seeded position, unlike every other style's
        // smooth or evenly-symmetric silhouette. Reads as ink dropped in
        // water: off barely spreads; on reaches further and rougher.
        const n=ring.length;
        if(n<3) return {glow:"", edge:""};
        const seed=hx*0.7+hy*1.3;
        const amt = on ? (0.22+0.18*t) : (0.08+0.06*t);
        let bcx=0, bcy=0;
        for(const [px,py] of ring){ bcx+=px; bcy+=py; }
        bcx/=n; bcy/=n;
        const bled=ring.map(([px,py],i)=>{
          const ang=i/n*Math.PI*2;
          const wob=Math.sin(ang*3+seed)*0.6+Math.sin(ang*7+seed*1.7)*0.3+Math.sin(ang*13+seed*2.3)*0.15;
          const f=1+amt*wob;
          return [bcx+(px-bcx)*f, bcy+(py-bcy)*f];
        });
        let bd="";
        for(let i=0;i<n;i++) bd += (i===0?"M":"L")+bled[i][0].toFixed(1)+","+bled[i][1].toFixed(1)+" ";
        bd+="Z";
        const fillOp=opac((on?0.24:0.13)+0.20*t);
        const edgeOp=opac((on?0.55:0.30)+0.25*t);
        const wash=`<path d="${bd}" fill="${duo}" fill-opacity="${fillOp}" pointer-events="none"/>`;
        const edge=`<path d="${bd}" fill="none" stroke="${ink}" stroke-width="${swid(on?1.3:0.9)}" stroke-opacity="${edgeOp}" stroke-linejoin="round" pointer-events="none"/>`;
        return {glow: clipWrap(`<g filter="url(#psaurasoft)">${wash}</g>`), edge: clipWrap(edge)};
      }
      if(AUTOMORPH_STYLE==="rosette"){
        // A symmetric ring of overlapping lens-shaped petals — a mandala
        // motif, never an organic or angular silhouette. Petal count and
        // reach carry state: off is a tight, few-petalled bud; on is a
        // fuller, wider-spread bloom.
        let maxR=20;
        for(const [px,py] of ring) maxR=Math.max(maxR, Math.hypot(px-hx,py-hy));
        const petalCount = on ? 8 : 6;
        const reach = maxR*(on?(0.55+0.30*t):(0.30+0.15*t));
        const petalW = reach*(on?0.42:0.32);
        const fillOp=opac((on?0.20:0.11)+0.16*t);
        const strokeOp=opac((on?0.55:0.30)+0.25*t);
        const petalD=(ang)=>{
          const tipx=hx+reach*Math.cos(ang), tipy=hy+reach*Math.sin(ang);
          const px=-Math.sin(ang), py=Math.cos(ang);
          const c1x=hx+petalW*px+reach*0.5*Math.cos(ang), c1y=hy+petalW*py+reach*0.5*Math.sin(ang);
          const c2x=hx-petalW*px+reach*0.5*Math.cos(ang), c2y=hy-petalW*py+reach*0.5*Math.sin(ang);
          return `M${hx.toFixed(1)},${hy.toFixed(1)} Q${c1x.toFixed(1)},${c1y.toFixed(1)} ${tipx.toFixed(1)},${tipy.toFixed(1)} `+
            `Q${c2x.toFixed(1)},${c2y.toFixed(1)} ${hx.toFixed(1)},${hy.toFixed(1)} Z`;
        };
        let petals="", outlines="";
        for(let k=0;k<petalCount;k++){
          const ang=(k/petalCount)*Math.PI*2 - Math.PI/2;
          const pd=petalD(ang);
          petals+=`<path d="${pd}" fill="${duo}" fill-opacity="${fillOp}" pointer-events="none"/>`;
          outlines+=`<path d="${pd}" fill="none" stroke="${ink}" stroke-width="${swid(on?1.0:0.7)}" stroke-opacity="${strokeOp}" pointer-events="none"/>`;
        }
        return {glow: clipWrap(petals), edge: clipWrap(outlines)};
      }
      if(AUTOMORPH_STYLE==="lensflare"){
        // A receding chain of translucent discs along one axis away from
        // the fixture — a photographic lens-flare/bokeh trail, never a
        // bounded shape wrapped around the fixture at all. Off is a
        // short, faint trail; on reaches further with more, brighter
        // discs. The axis is the fixture's own seeded angle, not "toward
        // the ring's centroid" — a fixture sitting near its room's own
        // centre (a common case: one fixture, one room) put that centroid
        // right on top of hx,hy, collapsing the whole chain to a single
        // point indistinguishable from a plain blob.
        const n=ring.length;
        let maxR=20;
        for(const [px,py] of ring) maxR=Math.max(maxR, Math.hypot(px-hx,py-hy));
        const seed=hx*0.7+hy*1.3;
        const ang=seed*0.6180339887; // golden-angle-ish scatter, deterministic per fixture
        const ux=Math.cos(ang), uy=Math.sin(ang);
        const discCount = on ? 5+Math.round(2*t) : 3;
        const reachMul = on ? (0.55+0.35*t) : (0.30+0.20*t);
        let discs="";
        for(let k=1;k<=discCount;k++){
          const frac=k/(discCount+1);
          const px=hx+ux*maxR*frac*reachMul, py=hy+uy*maxR*frac*reachMul;
          const r=(on?3.5:2.2)*(1-frac*0.4)+1.5*t;
          const op=opac((on?0.30:0.16)*(1-frac*0.5)+0.10*t);
          discs+=`<circle cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="${r.toFixed(2)}" fill="${duo}" fill-opacity="${op}" pointer-events="none"/>`;
        }
        const coreOp=opac((on?0.55:0.30)+0.25*t);
        const core=`<circle cx="${hx.toFixed(1)}" cy="${hy.toFixed(1)}" r="${swid(on?3:2)}" fill="${ink}" fill-opacity="${coreOp}" pointer-events="none"/>`;
        return {glow: clipWrap(`<g filter="url(#psaurasoft)">${discs}</g>`), edge: clipWrap(core)};
      }
      if(AUTOMORPH_STYLE==="mycelium"){
        // A branching filament network reaching from the fixture toward
        // several points around the ring — no fill, no bounded shape at
        // all, unlike every style above it. Off is a few short, thin,
        // unbranched threads; on is more, longer threads that fork
        // partway out — the one channel the lit state gets that off never
        // does, rather than a hue swap.
        const n=ring.length;
        if(n<3) return {glow:"", edge:""};
        const branchCount = on ? 6+Math.round(2*t) : 4;
        const step=Math.max(1, Math.floor(n/branchCount));
        const reach = on ? (0.75+0.25*t) : (0.40+0.20*t);
        const strokeOpRaw=(on?0.50:0.28)+0.25*t;
        const strokeOp=opac(strokeOpRaw);
        // Wider than a first pass tried (0.65-0.9px): at real map-marker
        // scale a sub-1px thread reads as nothing at all, not as "thin" —
        // this is the one style whose whole language is line-thinness, so
        // it has to spend its opacity/weight budget on staying visible
        // ahead of style flavour.
        const w=swid(on?1.6:1.1);
        let lines="";
        for(let k=0;k<branchCount;k++){
          const i=(k*step)%n;
          const [rx,ry]=ring[i];
          const endx=hx+(rx-hx)*reach, endy=hy+(ry-hy)*reach;
          const midx=hx+(rx-hx)*reach*0.55, midy=hy+(ry-hy)*reach*0.55;
          lines+=`<line x1="${hx.toFixed(1)}" y1="${hy.toFixed(1)}" x2="${midx.toFixed(1)}" y2="${midy.toFixed(1)}" `+
            `stroke="${ink}" stroke-width="${w}" stroke-opacity="${strokeOp}" stroke-linecap="round" pointer-events="none"/>`;
          lines+=`<line x1="${midx.toFixed(1)}" y1="${midy.toFixed(1)}" x2="${endx.toFixed(1)}" y2="${endy.toFixed(1)}" `+
            `stroke="${ink}" stroke-width="${swid(on?1.2:0.85)}" stroke-opacity="${strokeOp}" stroke-linecap="round" pointer-events="none"/>`;
          if(on){
            const perpX=-(ry-hy), perpY=(rx-hx);
            const plen=Math.hypot(perpX,perpY)||1;
            const forkx=midx+(perpX/plen)*8*(k%2?1:-1), forky=midy+(perpY/plen)*8*(k%2?1:-1);
            lines+=`<line x1="${midx.toFixed(1)}" y1="${midy.toFixed(1)}" x2="${forkx.toFixed(1)}" y2="${forky.toFixed(1)}" `+
              `stroke="${ink}" stroke-width="${swid(0.9)}" stroke-opacity="${opac(strokeOpRaw*0.7)}" stroke-linecap="round" pointer-events="none"/>`;
          }
        }
        return {glow: "", edge: clipWrap(lines)};
      }
    ...
    for(const r of hereRooms){
      ...
      // drawn through the name.
      {
        ...
        const near=(ly)=>hereLights.some(l=>{
          ...
          // Same footprint problem, one class down: a live humidity readout
          // is digits too, not a dot.
          const fresh=(lb&&(lb.isTemp||lb.isHumidity)&&lb.last_changed) ? NOW_MS-Date.parse(lb.last_changed) : NaN;
          const readout=!!(lb&&(lb.isTemp||lb.isHumidity)&&Number.isFinite(lb.isTemp?lb.temperature:lb.humidity)&&fresh>=0&&fresh<TEMP_FRESH_MS);
          const digits=lb&&lb.isTemp?lb.temperature:lb&&lb.humidity!=null?`${lb.humidity}%`:"";
          const halfW=readout ? String(digits).length*TEMP_DIGIT_PX*0.3 : 0;
    ...
    // that has the lock").
    {
      ...
      for(const bar of ((model && model.rf_barriers_m) || [])){
        ...
        if(BARRIER_HIT && dl){
          ...
          // data-invert/data-name/data-opener/data-lock carry enough of the
          // BARRIER's own fields (not just the sensor entity's) for a click
          // handler to reconstruct the same object this pass just drew from,
          // without a second model lookup — invert_state, and now the
          // opener/lock links, live on the barrier, invisible to anything
          // keyed on the sensor's eid alone (2026-09-22: openBarrierCard's
          // opener/lock sections silently never showed from this hit-path
          // until these two joined data-invert/data-name here — found live,
          // not guessed, clicking a real linked opener on the Atlas sidebar).
          s+=`<polyline class="lbarhit" data-eid="${escSVG(bar.linked_entity_id)}" data-cx="${mx.toFixed(1)}" data-cy="${my.toFixed(1)}" `+
            ...
            `data-opener="${escSVG(bar.linked_opener_entity_id||"")}" data-lock="${escSVG(bar.linked_lock_entity_id||"")}" `+
