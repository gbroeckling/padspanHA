...
const { C, S, h, check, select, numberBox, errText } = await import(`./wled_ui.js${_q}`);
...
function liveSyncCard(ctx) {
  ...
  const write = (patch) => ctx.write({ udpn: { send: !!u.send, sgrp: u.sgrp || 0, rgrp: u.rgrp || 0, ...patch } }, "change sync");
  card.appendChild(h("div", { style: "display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:4px 0" }, [
    ...
    h("span", { style: `font-size:12px;color:${C.dim}` }, "to groups"), groupChips(u.sgrp, v => write({ sgrp: v })),
  ]));
  card.appendChild(h("div", { style: "display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:4px 0" }, [
    h("span", { style: `font-size:12px;color:${C.dim};margin-right:8px` }, "Follow changes from groups"),
    groupChips(u.rgrp, v => write({ rgrp: v })),
  ]));
  card.appendChild(h("div", { style: `font-size:11px;color:${C.faint};margin-top:4px` },
    "These take effect now and last until the device restarts — save them below to keep them."));
  return card;
}
...
async function savedSyncCard(ctx, host) {
  ...
  if (ctx.isAdmin) card.appendChild(h("button", { style: S.btnPrimary + ";margin-top:8px", onclick: async () => {
    try {
      await ctx.call("padspan_ha/wled_cfg", { patch: { if: { sync: { send, recv } } }, base_hash: hash });
      ctx.toast("Sync settings saved (a backup was taken first)");
...
async function teamCard(ctx, host) {
  ...
  if (mine) {
    ...
    if (ctx.isAdmin) card.appendChild(h("button", { style: S.btn + `;color:${C.red};margin-top:8px`, onclick: async () => {
      if (!confirm(`Break up "${mine.name}"? Each device stops sending to / following sync group ${mine.group}.`)) return;
      await applyTeam(ctx, mine, devices, card, false);
      try {
        await ctx.hass.callWS({ type: "padspan_ha/wled_teams_set", teams: teams.filter(t => t !== mine) });
        ctx.toast("Team removed"); teamCard(ctx, host);
  ...
  card.appendChild(h("div", { style: `font-size:12px;color:${C.dim};margin-bottom:8px` },
    "Make this device the leader and pick the devices that should do what it does. PadSpan sets a WLED sync group on each "
    + "(backing each up first), and Home Assistant then only needs this light."));
  ...
  const draft = { name: `${me.name} team`, group: GROUPS.find(g => !usedGroups.has(g)) || 1, followers: new Set() };
  ...
  card.appendChild(h("div", { style: "display:flex;gap:8px;align-items:center;flex-wrap:wrap" }, [
    ...
    select(GROUPS.map(g => [g, usedGroups.has(g) ? `${g} (used)` : String(g)]), draft.group, v => { draft.group = Number(v); }),
  ]));
  card.appendChild(h("button", { style: S.btnPrimary + ";margin-top:8px", onclick: async () => {
    ...
    const team = { id: `team-${me.device_id.slice(0, 8)}`, name: draft.name, mode: "mirror", group: draft.group,
      leader: me.device_id, followers: [...draft.followers] };
    if (!confirm(`Set up "${team.name}": ${me.name} leads, ${team.followers.map(nameOf).join(", ")} follow, on sync group ${team.group}? Each device is backed up first.`)) return;
    const ok = await applyTeam(ctx, team, devices, card, true);
    if (!ok) return;
    try {
      await ctx.hass.callWS({ type: "padspan_ha/wled_teams_set", teams: [...teams, team] });
...
/** Set (or clear) the team's group on every member: saved config + live. */
async function applyTeam(ctx, team, devices, card, join) {
  const bit = M.maskOf([team.group]);
  const log = h("div", { style: `font-size:12px;margin-top:8px` });
  card.appendChild(log);
  const nameOf = (id) => (devices.find(d => d.device_id === id) || {}).name || id;
  let failed = 0;
  for (const [id, role] of [[team.leader, "leader"], ...team.followers.map(f => [f, "follower"])]) {
    ...
    try {
      ...
      const send = { ...(sync.send || {}) }, recv = { ...(sync.recv || {}) };
      const gen15 = M.wledGen(info) >= 15;
      if (role === "leader") {
        send.grp = join ? ((send.grp || 0) | bit) : ((send.grp || 0) & ~bit);
        if (join) { send.dir = true; if (gen15) send.en = true; }
      } else {
        recv.grp = join ? ((recv.grp || 0) | bit) : ((recv.grp || 0) & ~bit);
        if (join) { recv.bri = true; recv.col = true; recv.fx = true; if (gen15) recv.pal = true; }
      }
      await deviceCfgWrite(ctx, id, { if: { sync: role === "leader" ? { send } : { recv } } });
      // …and live, so it works now, not only after a restart.
      const live = role === "leader" ? { send: join || !!(send.en ?? send.dir), sgrp: send.grp } : { rgrp: recv.grp };
      await deviceState(ctx, id, { udpn: live });
  ...
  if (failed) ctx.toast(`${failed} device${failed === 1 ? "" : "s"} couldn't be set — see the list`, true);
  return failed === 0;
}
