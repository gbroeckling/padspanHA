...
### Traceback
- **New:** Full house activity has two modes. On its own it is your Atlas map with the tracked beacons playing back on it — no devices. Turn on **💡 Devices** (shown next to it) and the whole house plays back too: every device the Atlas shows, as it was through the chosen period — lights, doors, windows, locks, motion, and temperature, humidity and air-quality readings (a reading counts when what the map shows changes). Every tracked object plays along; each device that changed at the moment on screen is ringed on the map and named above it.
